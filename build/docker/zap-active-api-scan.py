...
def main(argv):
  ...
  try:
    opts, args = getopt.getopt(argv,"t:c:u:g:m:n:r:w:x:l:daijp:sz:P:D:", ['active_scan'])
  ...
  if running_in_docker:
    ...
    try:
      ...
      ls_output = subprocess.check_output(['docker', 'pull', 'stephendonner/zaproxy'])
    except OSError:
  ...
  try:
    ...
    if active_scan:
        ...
        # Give the active scan a change to start
        time.sleep(5)
        ...
        while (int(zap.ascan.status(ascan_scan_id)) < 100):
            if (datetime.now() - start).seconds > ((mins * 60) + 10):
                break
            print 'Active scan progress %: ' + zap.ascan.status(ascan_scan_id)
